...
const mostBlogs = blogsList => {
  ...
  const topMost = { author: max[0], blogs: max[1] }
  return topMost
}
...
const mostLikes = blogsList => {
  const map = {
    add([key, val]) {
      const oldVal = this[key] ? this[key] : 0
      this[key] = oldVal + val
    },
    ...
    max() {
      ...
      const [index] = vals.reduce((p, c, indx) => (typeof c === 'number' && c > p[1] ? [indx, c] : p), [0, -1])

      return index === -1 ? { author: '', likes: 0 } : { author: keys[index], likes: vals[index] }
    },
  }
...
const mostLikes2 = blogsList => {
  ...
  const { maxAuthor, maxLikes } = blogs.reduce((acc, crnt) => {
    const accLikes = acc[crnt.author] ? acc[crnt.author] : 0
