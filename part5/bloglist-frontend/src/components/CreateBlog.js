...
const CreateBlog = ({ blog, toggleBox, addUpdate }) => {
  ...
  const [inputs, setInput] = useState(constructNotNullObj(blog))
  ...
  if (blog?.title && inputs.title.length > 0 && blog.title !== inputs.title)
    setInput(constructNotNullObj(blog))
